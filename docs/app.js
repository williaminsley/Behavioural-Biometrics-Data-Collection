...
function showResults() {
  ...
  uploadSessionToFirebase(session)
    .then(() => {
      UI.uploadStatus().textContent =
        "Uploaded ✅ Thanks — play again later on the same device.";
      if (failBtn) failBtn.style.display = "none";
    })
    .catch((e) => {
      console.error(e);
      ...
      UI.uploadStatus().textContent =
        "Upload failed — taking you to downloads now.";

      if (failBtn) failBtn.style.display = "inline-block";

      // Immediately show the download screen
      setScreen("screen-data");
      renderSessionReport(UI.dataSummary(), session);
    });
